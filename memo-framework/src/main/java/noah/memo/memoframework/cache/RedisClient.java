package noah.memo.memoframework.cache;

import com.alibaba.fastjson.JSONObject;
import com.mysql.cj.util.StringUtils;
import noah.memo.memoframework.bean.FastDateFormat;
import noah.memo.memoframework.exception.RedisException;
import noah.memo.memoframework.log.Logger;
import noah.memo.memoframework.utils.FastDateUtils;
...
public class RedisClient {
    ...
    private static final String flagKeyFmt = "${key}_fast_flag_key";
    ...
    // 最大连接数
    private static final int MAX_TOTAL_COUNT = 50;
    ...
    public static String getFlagKey(String key) {
        return $format(flagKeyFmt, k_key, key);
    }
    ...
    public static String format(String text, String replacement, String searchString) {
        ...
        if (StringUtils.isNullOrEmpty(text) || StringUtils.isNullOrEmpty(searchString)) {
            return text;
        }

        replacement = replacement == null ? org.apache.commons.lang3.StringUtils.EMPTY : replacement;
     ...
     * @return
     */
    public static boolean existsFlagKey(String key) {
        return existsKey(getFlagKey(key));
    }
     ...
     * @param map
     */
    public static <T> void addToHashMapAndSetFlagKey(String hashKey, Map<String, T> map) {
        ...
        try {
            ...
            // 是否设置flagKey
            jedis.set(getFlagKey(hashKey), String.valueOf(System.currentTimeMillis()));
            ...
            closeResource(jedis);
            // 如果发生异常则把key及flagKey数据异常，调用获取数据方应当重新从数据库获取
            if (error) delKeyAndFlagKey(hashKey);
        }
    }
     ...
     * @param value
     */
    public static <T> void addToHashMapAndSetFlagKey(String hashKey, String key, T value) {
        ...
        try {
            ...
            // 是否设置flagKey
            if (!jedis.exists(hashKey)) jedis.set(getFlagKey(hashKey), String.valueOf(System.currentTimeMillis()));
    ...
    public static void removeHashMap(String hashKey) {
        if (StringUtils.isNullOrEmpty(hashKey)) {
            return;
        }
